...
| H14 | Type and navigate category suggestions in Add/Edit transaction | A new form starts with an empty field and closed suggestion list; after typing, the best match is highlighted first, followed by matching recent paths and alphabetical remaining paths; Up/Down moves the highlight without changing typed text, and Tab accepts the highlighted suggestion; free text creates a top-level category or Category / Subcategory on save; a failed transaction leaves no new categories |
...
| H22 | View Trend without type/category restrictions, then choose day/week/month grouping | Income is positive, expenses negative, Savings equals income minus expenses and may be negative; buckets cover the selected inclusive range with zero gaps; tooltips show exact values |
