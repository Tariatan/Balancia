...
| H26 | Check parent, child, and unrelated categories in the Categories panel | Child rows indent 5 pixels; selections combine across all flow/history panels, overlap is counted once, other filters remain active, and no checks means all categories |
| H27 | Switch between checkbox selection, category dropdown, and Clear filters | Both controls remain synchronized; dropdown replaces the set, clear removes all checks, and double-click on a category name still edits it |
...
| H09 | Resize Windows app from its 1280 × 1280 minimum | Overview history and Upcoming payments reach the same bottom edge without an outer scrollbar; the reminders list expands with the window; bold history amounts show green income, red expense, dark blue transfer |
...
| H14 | Type and navigate category suggestions in Add/Edit transaction | A new form starts with an empty field and closed suggestion list; after typing, the best match is highlighted first, followed by matching recent paths and alphabetical remaining paths; Up/Down moves the highlight without changing typed text, and Tab accepts the highlighted suggestion; saving an unknown path is rejected and never creates a category; existing paths save case-insensitively, blank means Uncategorized, and a failed transaction leaves the ledger unchanged |
...
| H21 | View Upcoming payments with reminders across multiple months | TOTAL NET equals the sum of all displayed reminder amounts; TOTAL UPCOMING MONTH equals the sum of displayed reminders due in the next local calendar month, including across a year boundary |
| H22 | View Trend without type/category restrictions, then use the mouse wheel to change day/week/month grouping | Wheel up advances Day → Week → Month and stops at Month; wheel down reverses and stops at Day; titles/descriptive captions are absent; Income is positive, expenses negative, Savings equals income minus expenses and may be negative; buckets cover the selected inclusive range with zero gaps; tooltips show exact values |
| H23 | Combine Overview filters and page through history | Both charts use every matching transaction, include child categories, and refresh in place with filters; changing history page does not change chart totals; opening balances and transfers never enter flow totals |
| H24 | Compare Timeline across month/year/leap-day boundaries | Current and preceding equal-length ranges retain the same non-date filters, accumulate separately, and align by elapsed days; line plateaus and tooltip values reflect only posted transactions |
| H25 | Show empty, single-day, income-only category, expense-only category, and transfer-only results | Charts remain usable, single-series filters use the corresponding metric, and empty/transfer-only results have an explanatory state |
